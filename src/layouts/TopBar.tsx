  ...
  return (
    <div className="h-[var(--header-height)] bg-background-elevated border-b border-[var(--border-subtle)] flex items-center px-3">
      <div className="flex-1 flex items-center gap-2">
        ...
        <Tooltip content={
          <div>
            <div>{isPresentMode ? "Stop presentation" : "Start broadcasting to all outputs"}</div>
            <div className="keyboard-shortcut"><kbd>F5</kbd></div>
          </div>
        } position="bottom">
          <button 
          ...
          >
            ...
            <span className="text-sm font-medium">{isPresentMode ? 'Stop' : 'Go Live'}</span>
          </button>
        </Tooltip>
        
        {/* AI Assistant button */}
        <Tooltip content={<div>AI-powered suggestions<div className="keyboard-shortcut"><kbd>⌥A</kbd></div></div>} position="bottom">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-background-glass/10">
          <div className={`w-2 h-2 ${liveColor} rounded-full ${isPresentMode ? 'animate-pulse' : ''}`} />
          <span className="text-sm text-text-primary font-medium">Sunday Service</span>
        ...
        <div className="ml-auto flex items-center gap-1">
          <Tooltip content={<div>Save service<div className="keyboard-shortcut"><kbd>⌘S</kbd></div></div>} position="bottom">
            <button className="p-1.5 rounded-md hover:bg-black/5">
              <Save className="w-4 h-4 text-text-secondary" />
            </button>
          </Tooltip>
          
          <Tooltip content={<div>Toggle fullscreen<div className="keyboard-shortcut"><kbd>F11</kbd></div></div>} position="bottom">
            <button 
          ...
          <Tooltip content={<div>Keyboard shortcuts<div className="keyboard-shortcut"><kbd>?</kbd></div></div>} position="bottom">
            <button 
      ...
      <div className="flex items-center gap-2 ml-2">
        ...
        <Tooltip content={helpMode ? "Disable help mode" : "Enable help mode"} position="bottom">
          <button 
            onClick={toggleHelpMode}
            className={`p-1.5 rounded-md ${helpMode ? 'bg-accent-blue/20' : 'hover:bg-background-glass/10'}`}
          >
            <HelpCircle className={`w-4 h-4 ${helpMode ? 'text-accent-blue' : 'text-text-secondary'}`} />
          </button>
        </Tooltip>
        
        <Tooltip content={isLightMode ? "Switch to dark mode" : "Switch to light mode"} position="bottom">
          <button 
            onClick={toggleTheme}
            className="p-1.5 rounded-md hover:bg-background-glass/10"
          >
            {isLightMode ? <Moon className="w-4 h-4 text-text-secondary" /> : <Sun className="w-4 h-4 text-text-secondary" />}
          </button>
        </Tooltip>
      </div>
