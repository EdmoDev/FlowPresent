...
import { ChevronRight, Timer, PlusCircle, GripVertical, Edit2, Trash2, Copy, ChevronDown, ChevronUp } from 'lucide-react';
...
const serviceTimeline: TimelineItem[] = [
  ...
  { id: '3', name: 'Worship Set', duration: '20:00', status: 'upcoming', items: ['Amazing Grace', 'How Great Thou Art'] },
    ...
    <div className="space-y-0.5">
      ...
      <div className="space-y-1.5 max-h-[calc(100vh-14rem)] overflow-y-auto pr-1 custom-scrollbar">
        ...
        >
          ...
          <span className="text-sm font-medium font-['JetBrains Mono'] tracking-[-0.02em]">Add Service Item</span>
        </button>
      </div>
    </div>
  );
...
const TimelineItemCard: React.FC<TimelineItemCardProps> = ({ item, index, isCompact }) => {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <span className="text-text-primary truncate font-['JetBrains Mono'] tracking-[-0.02em]">{item.name}</span>
          <span className="text-text-secondary text-sm ml-2 font-['JetBrains Mono'] tracking-[-0.02em]">{item.duration}</span>
        </div>
        
        {!isCompact && item.items && (
          <div className="mt-2 pl-2 border-l-2 border-[var(--border-subtle)]">
            {item.items.map(subItem => (
              <div key={subItem} className="text-xs text-text-secondary py-0.5 flex items-center gap-1 font-['JetBrains Mono'] tracking-[-0.02em]">
