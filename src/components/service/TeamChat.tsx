    ...
    <div className="glass-panel relative">
      <div className="p-4 border-b border-[var(--border-subtle)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-medium text-text-primary font-['JetBrains Mono'] tracking-[-0.02em]">
            Team Chat
          </h3>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {/* Minimize button */}
          <Tooltip
            content={isCollapsed ? "Expand chat" : "Collapse chat"}
            position="top"
            alwaysShow={true}
          >
            <button
          ...
          <div className="flex flex-col h-[280px] p-4 flex-1">
            <div className="space-y-3 flex-1 overflow-y-auto pr-1 mb-3 custom-scrollbar">
              {messages.map(({ id, name, role, message, timestamp, isYou }) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center shadow-glass ${
                      isYou
                  ...
                  >
                    <span
                      className={`text-sm font-medium ${isYou ? "text-blue-400" : "text-purple-400"}`}
                    >
                      {name[0]}
                    </span>
                  </div>
                  ...
                  <div
                    className={`${isYou ? "items-end text-right" : ""} flex-1`}
                  >
                    <div
                      className={`flex items-baseline gap-2 ${isYou ? "flex-row-reverse" : ""}`}
                    >
                      <div
                        className={`flex items-baseline gap-2 ${isYou ? "flex-row-reverse" : ""}`}
                      >
                        <p className="text-sm font-medium text-text-primary font-['JetBrains Mono'] tracking-[-0.02em]">
                          {name}
                        </p>
                        <p className="text-xs text-text-secondary font-['JetBrains Mono'] tracking-[-0.02em]">
                          {role}
                        </p>
                      </div>
                      <p className="text-xs text-text-secondary font-['JetBrains Mono'] tracking-[-0.02em]">
                    ...
                    <div
                      className={`mt-0.5 ${isYou ? "bg-blue-500/10 ml-8" : "bg-[var(--background-glass)]/20 mr-8"} px-3 py-1.5 rounded-lg`}
                    >
                      <p className="text-sm text-text-secondary font-['JetBrains Mono'] tracking-[-0.02em]">
                        {message}
                      </p>
                    </div>
                  </div>
            ...
            <div className="pt-2 border-t border-[var(--border-subtle)]">
              <div className="flex gap-2">
                ...
                >
                  <Send
                    className={`w-4 h-4 ${newMessage.trim() ? "text-white" : "text-text-secondary"}`}
                  />
                </button>
              </div>
          ...
          {showUsers && (
            <div className="w-64 border-l border-[var(--border-subtle)] p-3">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-sm font-medium text-text-primary font-['JetBrains Mono'] tracking-[-0.02em]">
                  Team Members
                </h4>
                <button
              ...
              <div className="space-y-2 max-h-[230px] overflow-y-auto custom-scrollbar">
                {teamMembers.map((member) => (
                  <div
                    key={member.name}
                    className="flex items-center gap-2 p-1.5 hover:bg-background-glass/10 rounded-lg"
                  >
                    ...
                    <div>
                      <p className="text-sm text-text-primary font-['JetBrains Mono'] tracking-[-0.02em]">
                        {member.name}
                      </p>
                      <div className="flex items-center gap-1.5">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${member.online ? "bg-green-500" : "bg-gray-500"}`}
                        ></div>
                        <p className="text-xs text-text-secondary font-['JetBrains Mono'] tracking-[-0.02em]">
                          {member.role}
                        </p>
