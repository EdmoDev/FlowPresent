...
const Sidebar: React.FC = () => {
  ...
  return (
    <div
      className={`h-full glass-panel border-r border-[var(--border-subtle)] flex flex-col ${
    ...
    >
      ...
      <Tooltip content="ProFlow Church Presentation Software" position="right">
        <div
          className={`flex items-center gap-2 mb-6 ${sidebarCollapsed ? "justify-center" : ""}`}
        >
      ...
      {/* Main navigation */}
      <div
        className={`mb-4 border-b border-[var(--border-subtle)] pb-4 ${sidebarCollapsed ? "" : ""}`}
      >
        {!sidebarCollapsed && (
          <h2 className="text-xs uppercase text-text-secondary font-medium mb-2 px-2">
            Main
          </h2>
        )}
...
const NavButton: React.FC<NavButtonProps> = ({
  ...
  const ButtonContent = () => (
    <>
      ...
      {!collapsed && shortcut && !isActive && (
        <span className="ml-auto text-xs text-text-secondary opacity-60">
          {shortcut}
        </span>
      )}
      ...
      <Tooltip
        content={
          <div>
            {label}
            {shortcut && (
              <div className="mt-1 text-xs opacity-70">{shortcut}</div>
            )}
          </div>
        }
        position="right"
